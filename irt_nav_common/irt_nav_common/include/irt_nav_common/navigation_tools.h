...
#ifndef IRT_NAV_COMMON_NAVIGATION_TOOLS_H
#define IRT_NAV_COMMON_NAVIGATION_TOOLS_H
...
namespace irt_nav_common {
    ...
    inline gtsam::Point3 llh2xyz(const std::array<double, 3>& llh)
    {
      return llh2xyz(gtsam::Point3(llh[0], llh[1], llh[2]));
    }
    ...
    inline gtsam::Point3 ned2enu(const gtsam::Point3 &p1) {
    ...
    inline gtsam::Vector3 getMagOrientation(const gtsam::Vector3 &pos)
    {
      ...
      */
      ...
      gtsam::Vector3 posllh = xyz2llh(pos);
      gtsam::Vector3 magField_enu;
      GeographicLib::MagneticModel magMod("wmm2020");
      magMod(year, rad2deg * posllh.x(), rad2deg * posllh.y(), posllh.z(),
      ...
      return nedRe_Matrix(pos).transpose() * nedRenu.matrix() * magField_enu; //nanotesla
    }
...
#endif //IRT_NAV_COMMON_NAVIGATION_TOOLS_H

...
namespace fgo::integrator {
  ...
  public:
    ...
    bool checkStatePresentedInCurrentLag(size_t id) {
      if (lastLagKeys_.empty()) {
        RCLCPP_WARN_STREAM(rosNodePtr_->get_logger(), integratorName_ << " checking state presence for key: " << id
    ...
    static void setSensorFrameFromParam(const std::string &frameStr,
      ...
      if (frameStrLower == "body")
        ...
        RCLCPP_WARN_STREAM(rclcpp::get_logger("onlineFGO"),
                           type << " improper frame: " << frameStrLower << " setting as NED");
      }
    }
  ...
  protected:
    ...
    void addGPInterpolatedNavAttitudeFactor(const gtsam::Key &poseKeyI, const gtsam::Key &velKeyI,
      ...
      graphPtr_->emplace_shared<fgo::factor::GPInterpolatedNavAttitudeFactor>(poseKeyI, velKeyI, omegaKeyI, poseKeyJ,
                                                                              velKeyJ, omegaKeyJ,
                                                                              rotMeasured,
                                                                              integratorBaseParamPtr_->attitudeFrame,
                                                                              type,
    ...
    void initIntegratorBaseParams() {
      ...
      RCLCPP_INFO(rosNodePtr_->get_logger(),
                  "---------------------  IntegratorBase initialized! --------------------- ");
    }

    std::shared_ptr<fgo::models::GPInterpolator>
    maybeWrapNNGPResidualInterpolator(const std::shared_ptr<fgo::models::GPInterpolator> &baseInterpolator,
      ...
      if (integratorBaseParamPtr_->NNGPResidualModelPath.empty()) {
        RCLCPP_WARN_STREAM(rosNodePtr_->get_logger(),
                           ownerName << ": useNNGPResidual is true but NNGPResidualModelPath is empty. "
                                     << "Using base GP interpolator.");
        return baseInterpolator;
      }
      ...
      try {
        ...
        RCLCPP_ERROR_STREAM(rosNodePtr_->get_logger(),
                            ownerName << ": failed to load GP residual NN interpolator from "
                                      << integratorBaseParamPtr_->NNGPResidualModelPath << ": "
                                      << ex.what() << ". Using base GP interpolator.");
        return baseInterpolator;
      }
    }
  ...
  public:
    ...
    std::tuple<bool, gtsam::Vector6, bool, gtsam::Vector6> findAccelerationToState(size_t idStateI,
      ...
      if (!foundAccI || !foundAccJ)
        RCLCPP_WARN_STREAM(rosNodePtr_->get_logger(),
                           integratorName_ << " can find accI? " << foundAccI << " or accJ? " << foundAccJ);
    ...
    static StateMeasSyncResult findStateForMeasurement(const std::map<size_t, double> &varIDTimestampMap,
      ...
      if (varIDTimestampMap.begin()->second > correctedTimestampMeas &&
          (correctedTimestampMeas - varIDTimestampMap.begin()->second) < paramPtr->StateMeasSyncLowerBound) {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("online_fgo"), "Measurement at: " << std::fixed <<
                                                                                correctedTimestampMeas
                                                                                << " is out of the current lag with earliest state: "
                                                                                << varIDTimestampMap.begin()->second);
        result.status = StateMeasSyncStatus::DROPPED;
        return result;
      }
      ...
      while (varIDTimeIter != varIDTimestampMap.rend()) {
        ...
        if (varIDTimeIter == varIDTimestampMap.rbegin() && varGNSSdt > paramPtr->StateMeasSyncUpperBound) {
          ...
          RCLCPP_WARN_STREAM(rclcpp::get_logger("online_fgo"), "findStateForMeasurement: " << std::fixed <<
                                                                                           " Measurement at "
                                                                                           << correctedTimestampMeas
                                                                                           << " in front of LAST state at "
                                                                                           << varIDTimeIter->second

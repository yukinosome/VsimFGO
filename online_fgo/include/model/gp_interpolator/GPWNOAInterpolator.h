...
#ifndef ONLINE_FGO_GPWNOAINTERPOLATOR_H
#define ONLINE_FGO_GPWNOAINTERPOLATOR_H
...
namespace fgo::models {
  ...
  class GPWNOAInterpolator : public GPInterpolator {

  private:
    typedef GPWNOAInterpolator This;
  ...
  public:
    ...
    GPWNOAInterpolator() : GPInterpolator() {}
     ...
     */
    explicit GPWNOAInterpolator(const gtsam::SharedNoiseModel &Qc_model, double delta_t = 0.0,
    ...
    GPWNOAInterpolator(const This &interpolator) :
    ...
    /** Virtual destructor */
    ~GPWNOAInterpolator() override = default;
    ...
    [[nodiscard]] gtsam::Pose3 interpolatePose(
      ...
      if (useAutoDiff_) {
        ...
        Vector6 r;
        if (H1 || H4)
          // We calculate the local increment between T1 and T2, here we calculate it other way round as in Barfoot's paper
          // which means we have the right Jacobian instead of the left Jacobian
          // ori math: xi_i = In(T_jT_i^-1)^vee  our. math xi_i = In(T_i^-1T_j)^vee
          // by doing so, the math to compute the pose = pose_t = pose_i * exp(...) instead of pose_t = exp(...) * pose_i
        ...
        if (H1 || H2 || H3 || H4 || H5 || H6) {
          ...
          pose = pose1.compose(Pose3::Expmap(Lambda_.block<6, 12>(0, 0) * r1 + Psi_.block<6, 12>(0, 0) * r2, Hlogmap2),
                               Hcomp21, Hcomp22);
          ...
          if (H1) {
            ...
            if (calcJacobian_) {
              Matrix_12_6 dr2_dT1 = (Matrix_12_6() << JTi, jacobianMethodNumercialDiff(rightJacobianPose3inv, r, vel2) *
                                                           JTi).finished();
          ...
          if (H4) {
            ...
            if (calcJacobian_) {
              const Matrix_12_6 dr2_dT2 = (Matrix_12_6() << JTj,
                jacobianMethodNumercialDiff(rightJacobianPose3inv, r, vel2) * JTj).finished();
      ...
      if (useAutoDiff_) {
        if (H1)
          *H1 = gtsam::numericalDerivative11<gtsam::Vector6, gtsam::Pose3>(
            boost::bind(&This::interpolateVelocity_, this, boost::placeholders::_1, v1_n, omega1_b, pose2, v2_n,
                        omega2_b),
            pose1, 1e-5);

        if (H2)
          *H2 = gtsam::numericalDerivative11<gtsam::Vector6, gtsam::Vector3>(
            boost::bind(&This::interpolateVelocity_, this, pose1, boost::placeholders::_1, omega1_b, pose2, v2_n,
                        omega2_b),
            v1_n, 1e-5);
        ...
        if (H4)
          *H4 = gtsam::numericalDerivative11<gtsam::Vector6, gtsam::Pose3>(
            boost::bind(&This::interpolateVelocity_, this, pose1, v1_n, omega1_b, boost::placeholders::_1, v2_n,
                        omega2_b),
            pose2, 1e-5);

        if (H5)
          *H5 = gtsam::numericalDerivative11<gtsam::Vector6, gtsam::Vector3>(
            boost::bind(&This::interpolateVelocity_, this, pose1, v1_n, omega1_b, pose2, boost::placeholders::_1,
                        omega2_b),
            v2_n, 1e-5);
        ...
        const gtsam::Vector6 xi_I_tau =
          Lambda_.block<6, 6>(0, 6) * vel1 + Psi_.block<6, 6>(0, 0) * r + Psi_.block<6, 6>(0, 6) * Jinv * vel2;
        const gtsam::Vector6 xi_J_tau =
          Lambda_.block<6, 6>(6, 6) * vel1 + Psi_.block<6, 6>(6, 0) * r + Psi_.block<6, 6>(6, 6) * Jinv * vel2;
        const gtsam::Matrix6 Jac_Xi_I_tau = fgo::utils::leftJacobianPose3(
          xi_I_tau);  // here we must use the left Jacobian because we are not inverting the calculation
        ...
        if (H1 || H2 || H3 || H4 || H5 || H6) {
          ...
          if (calcJacobian_) {
            JinvJacobianVel2 = 0.5 * fgo::utils::curlyhat(vel2) *
                               Jinv; //jacobianMethodNumercialDiff(rightJacobianPose3inv, r, vel2); we changed back to Barfoot's math using the transformation x^y = -y^x ^: curlyhat
            DJinvJacobianVel2 =
          ...
          if (H1) {  // Shape:  6 x 6
            ...
            if (calcJacobian_) {
              Matrix_12_6 dvel_dr = (Matrix_12_6() << Jinv * JTi, JinvJacobianVel2 * JTi).finished();
              *H1 = Jac_Xi_I_tau * Psi_.block<6, 12>(6, 0) * dvel_dr +
                    DJinvJacobianVel2 * Psi_.block<6, 12>(0, 0) * dvel_dr + Hvel1 * H1p;
            } else
          ...
          if (H4) {
            ...
            if (calcJacobian_) {
              ...
              *H4 = Jac_Xi_I_tau * Psi_.block<6, 12>(6, 0) * dvel_dr +
                    DJinvJacobianVel2 * Psi_.block<6, 12>(0, 0) * dvel_dr + Hvel2 * H2p;
            } else
    ...
    void print(const std::string &s = "") const override {
      std::cout << s << "GPWNOAInterpolator" << std::endl;
...
namespace gtsam {
  template<>
  struct traits<fgo::models::GPWNOAInterpolator> : public Testable<
    fgo::models::GPWNOAInterpolator> {
  };
}

#endif //ONLINE_FGO_GPWNOAINTERPOLATOR_H

...
namespace fgo {
  namespace models {
    class GPExtrapolatorPose3Singer {

    private:
      typedef GPExtrapolatorPose3Singer This;
      fgo::utils::Matrix_18 Phi_;

    public:
      /// Default constructor: only for serialization
      GPExtrapolatorPose3Singer() {}

      GPExtrapolatorPose3Singer(const gtsam::Matrix6 Ad, double tau) {
        // Calcuate phi
        Phi_ = fgo::utils::calcPhiSinger<6>(Ad, tau);
      }

      /** Virtual destructor */
      virtual ~GPExtrapolatorPose3Singer() {}
      ...
      void print(const std::string &s = "") const {
        std::cout << s << "GaussianProcessExtrapolatorPose3Singer" << std::endl;

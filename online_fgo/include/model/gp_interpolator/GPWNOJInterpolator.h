...
#ifndef ONLINE_FGO_GPWNOJINTERPOLATOR_H
#define ONLINE_FGO_GPWNOJINTERPOLATOR_H
...
namespace fgo::models {
  class GPWNOJInterpolator : public GPInterpolator { //Translation world/earth Rotation body
  ...
  private:
    typedef GPWNOJInterpolator This;

  public:
    /// Default constructor: only for serialization
    GPWNOJInterpolator() : GPInterpolator() {}
     ...
     */
    explicit GPWNOJInterpolator(const gtsam::SharedNoiseModel &Qc_model, double delta_t = 0.0, double tau = 0.0,
                                bool useAutoDiff = false, bool calcJacobian = true, bool calcMatrices = true) :
    ...
    GPWNOJInterpolator(const This &interpolator) :
    ...
    /** Virtual destructor */
    ~GPWNOJInterpolator() override = default;
    ...
    [[nodiscard]] gtsam::Pose3 interpolatePose(
      ...
      if (useAutoDiff_) {
        ...
        if (H1 || H2 || H3 || H4 || H5 || H6) {
          ...
          const gtsam::Matrix6 A2SkewMatrix = fgo::utils::curlyhat(accJ_);
          Matrix6 Hcomp21, Hcomp22, Hlogmap2;

          const gtsam::Vector6 xi_i_tau = Lambda_.block<6, 18>(0, 0) * r1 + Psi_.block<6, 18>(0, 0) * r2;

          Matrix6 JXiTau = gtsam::Matrix6::Identity();
          if (calcJacobian_) {
            JXiTau = fgo::utils::leftJacobianPose3(xi_i_tau);
          }

          pose = pose1.compose(Pose3::Expmap(xi_i_tau, &Hlogmap2), &Hcomp21, &Hcomp22);

          Matrix6 Hexpr1 = Hcomp22 * Hlogmap2;  // dim 6x6
          Matrix6 Hvel1 = Hexpr1 * Lambda_.block<6, 6>(0, 6) * JXiTau;
          Matrix6 Hvel2 = Hexpr1 * (Psi_.block<6, 6>(0, 6) * JXiTau +
                                    (-0.5) * Psi_.block<6, 6>(0, 12) * JXiTau * (JinvVel2SkewMatrix - V2SkewMatrix)) *
                          Jinv;
          // Matrix6 Hacc1 = Hexpr1 * Lambda_.block<6, 6>(0, 12);

          if (H1) {
            const Matrix6 J_Ti = Hlogmap1 * Hcomp11 *
                                 Hinv;  // Jacobian of the log(T_i^-1*T_j), here this is slightly different to Barfoot's paper ln(T_j * T_i^-1)
            ...
            if (calcJacobian_) {
              ...
              *H1 = Hcomp21 +  // Jacobian of the expotential map while calculating interpolated pose at line 190
                    Hvel1 * H1p +
                    // Jacobian due to velocity frame transformation from global to local frame, how the velocity matters the pose
                    Hexpr1 * Psi_.block<6, 12>(0, 0) * dr_dT1 * J_Ti +
                    // dim 6 x 6 * 6 x 12 * 12 * 6 * 6 * 6 Jocibian while interpolating velocity, Eq. 5.77 of Tang's master thesis page 68
...
namespace gtsam {
  template<>
  struct traits<fgo::models::GPWNOJInterpolator> : public Testable<
    fgo::models::GPWNOJInterpolator> {
  };
}

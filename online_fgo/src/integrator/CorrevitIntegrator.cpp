...
namespace fgo::integrator
{
    ...
    bool
    CorrevitIntegrator::addFactors(const boost::circular_buffer<std::pair<double, gtsam::Vector3>> &timestampGyroMap,
...
/*
        else if (syncResult.status == StateMeasSyncStatus::INTERPOLATED && integratorParamPtr_->addGPInterpolatedFactor)
        {
          RCLCPP_WARN_STREAM(appPtr_->get_logger(),

...
namespace gnss_fgo {
  GNSSFGOBoreasNode::GNSSFGOBoreasNode(const rclcpp::NodeOptions &opt) :
    ...
    subPVAGT_ = this->create_subscription<nav_msgs::msg::Odometry>("/boreas/gps_gt",
                                                                   ...
                                                                   std::bind(&GNSSFGOBoreasNode::onPVAGTMsgCb, this,
                                                                             std::placeholders::_1));
  ...
  void GNSSFGOBoreasNode::onPVAGTMsgCb(nav_msgs::msg::Odometry::ConstSharedPtr pva) {
    ...
    referenceBuffer_.update_buffer(this_pva, ts);
  }
  ...
  void GNSSFGOBoreasNode::calculateErrorOnState(const fgo::data::State &stateIn) {
    ...
    auto pvtBuffer = referenceBuffer_.get_all_buffer();
    ...
    while (stateIter != stateCached.end()) {
      ...
      if (stateTime < lastPVTTime) {
        ...
        const auto coeff =
          (stateTime - itBefore->timestamp.seconds()) / (itAfter->timestamp.seconds() - itBefore->timestamp.seconds());
